...
7. 목표 정렬: **mascotSteering이 false일 때만 적용한다.** targetGoal이 있으면 candidateText가
   ...
   ※ 여기서 "질문"은 **글자 그대로 묻는 문장**만 말한다. 판단 기준은 하나다:
   candidateText 에 물음표가 있거나, 물음표 없이도 아이에게 답을 요구하는 표현
   ("말해 줘", "어떤지 알려 줘")이 있는가.
   서술문과 수긍은 질문이 아니다. "응.", "...응.", "그랬구나.", "그런 거였나.",
   "음... 그랬을 것 같아.", "뭔가 힘들어 보였어." 는 **전부 질문이 아니고 PASS다.**
   이런 문장을 두고 "묵시적으로 묻고 있다", "답을 유발한다", "마스코트 질문과
   순서가 겹친다"는 식으로 읽지 마라 — 실측에서 이 오판이 SCAFFOLD 후보의 38%를
   떨어뜨렸고, 그 18건 중 물음표가 있는 것은 하나도 없었다. 아이는 그 대가로
   사전 승인된 고정 문구를 받았다.
   CHARACTER_STEERS_ANYWAY 는 위 판단 기준에 실제로 걸릴 때만 쓴다.
...
9. 두 대사의 정합: mascotLine과 candidateText가 같은 장면에서 이어져야 한다. 마스코트가
   ...
   BOTH_ASK_QUESTIONS 도 기준 7의 "질문" 정의를 그대로 쓴다 — **두 대사 모두에**
   물음표나 답을 요구하는 표현이 실제로 있어야 한다. 한쪽이 서술문이면 해당 없다.
   그리고 "순서가 겹친다", "중첩된다"는 것은 감점 사유가 아니다. 마스코트가 묻고
   캐릭터가 짧게 반응하는 것은 설계된 모양이다.
...
그 후보는 통과시켜라.
  ...
  CHARACTER_STEERS_ANYWAY  (7) 마스코트가 유도를 맡은 턴인데 캐릭터도 질문한다
                               — candidateText 에 물음표나 답 요구 표현이 실제로 있을 때만
